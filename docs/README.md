...
> A: 
> 1. 免费，内置部分广告；广告是维持项目持续更新的一部分收入来源，请您理解。（[广告遮挡，已影响使用？](#QandA)）
> 2. 适配多端， 除了小程序外，同时还有桌面客户端（Windows & Mac）、快捷指令、 utools插件
> 3. 支持多种推送方式（聚合app 、微信、邮件推送）
> 4. 支持多种推送模式（固定链接、临时链接、订阅链接）
> 5. 支持授权关系，扩展小程序推送场景。
> 6. 使用简单；简单请求即可轻松实现
>
...
| 授权码     |第三方客户端用于获取授权取得账户权限的凭证   |登录验证码|
...
> Q: 广告设置不合理，遮挡到页面内容，影响使用

A: 

广告是维持项目持续更新的一部分收入来源，请您理解。广告作为页面的一部分，开发者一直希望将广告与页面更好融合，做到既不影响使用也可保持良好的广告阅读。如果广告遮挡了您的页面内容，您可截图此页面并通过[社区](https://support.qq.com/products/111465)进行反馈，开发者会尽快做出调整。 

如果您认为部分位置的广告设置虽然没有遮挡页面，但影响体验。也可通过[社区](https://support.qq.com/products/111465)进行反馈。

> Q: 有哪些获取奇点的方式？

A: 

目前赚取奇点的方式只提供了看激励视频获取的方式，您也可以通过[社区](https://support.qq.com/products/111465)反馈有价值的bug或建议来获取额外的奖励。
